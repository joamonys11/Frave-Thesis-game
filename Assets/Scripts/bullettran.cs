...
public class bullettran : MonoBehaviour {
	...
	void Start () {
		...
		shoot = GameObject.Find ("shoot").transform;
...
//		transform.position = Vector3.SmoothDamp (shoot.position, target2,ref target2, 100);
			//transform.position = Vector3.Lerp(shoot.position, target2,  fracComplete);
		Debug.Log("Shoot");

		Destroy (gameObject,3f);

		//Destroy (GameObject.Find(hitraycast.enemy), 10f);
